...
{
	pixmap pm(COLS*CHAR_WIDTH, ROWS*CHAR_HEIGHT, pixmap::pixel_type::GRAY);
...
void
debug_font::draw_string_fmt(float x, float y, const char *fmt, ...) const
{
	char buf[512];
...
{
	vertex_array<vertex_texuv> va(strlen(str)*4);
	...
	for (const char *p = str; *p; p++) {
		...
		const float x1 = x0 + CHAR_HEIGHT;

		const float y0 = y + .5*CHAR_HEIGHT;
		const float y1 = y0 - CHAR_HEIGHT;
